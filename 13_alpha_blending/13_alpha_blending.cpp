...
bool loadMedia()
{
    ...
    // load background
    if (gBackgroundImage.loadFromFile("background_image.png", gRenderer) == false)
    {
        printf("Failed to load background image texture. \n");
        loadMediaSuccess = false;
    }

    // load modulated image
    if (gModulatedImage.loadFromFile("modulated_image.png", gRenderer) == false)
    {
        printf("Failed to load modulated image texture. \n");
        loadMediaSuccess = false;
    }
    else
    {
        gModulatedImage.setBlendMode(SDL_BLENDMODE_BLEND);
    }
    ...
    else
    {
        ...
        else
        {
            ...
            while (!quit)
            {
                ...
                // draw
                gBackgroundImage.render(0, 0, gRenderer);
                gModulatedImage.setAlpha(alpha);
                gModulatedImage.render(0, 0, gRenderer);
